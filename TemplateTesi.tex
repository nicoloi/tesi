...
\label{app}
Il metodo di risoluzione ha diverse applicazioni pratiche, come nei campi dell'intelligenza artificiale, della verifica formale e dell'informatica teorica. Può essere utilizzato per la dimostrazione automatica di teoremi matematici e logici. L'utilizzo pratico che viene implementato nel nostro caso è quello di verificare la validità di formule logiche.
...
Per verificare la validità di una qualsiasi formula $F$, si considera $\lnot F$ e si controlla se è soddisfacibile o meno; se $\lnot F$ è \textbf{insoddisfacibile}, allora $F$ è valida. Questo perché, come spiegato nel capitolo \ref{intro}, se applichiamo la negazione logica ad una tautologia otteniamo una contraddizione, e viceversa.
...
\end{definition}
Con la verifica della validità delle formule, è possibile dimostrare la conseguenza logica di due formule $F_1$ e $F_2$ utilizzando l'implicazione logica, ovvero il connettivo $\to$. Infatti, se $F_2$ è conseguenza logica di $F_1$, allora la formula:
\[
    F_1 \to F_2
\]
è valida.
\subsubsection{Esempio di conseguenza logica}
Un esempio che possiamo considerare è quello del \emph{modus ponens}: siano $a, b$ due formule, se la formula $a \to b$ è vera, e anche $a$ è vera, allora la conseguenza $b$ è vera.
...
\[
    ...
    \DP
\]
Per dimostrare il modus ponens, Possiamo creare una formula $F$ utilizzando la congiunzione logica per mettere insieme le due premesse ($a$ e $a \to b$), e l'implicazione logica per rappresentare la conseguenza logica con $b$. La formula $F$ ottenuta è la seguente:
\[
    F \; = \; (a \land (a \to b)) \to b
\]
A questo punto, bisogna verificare che la formula $\lnot F$ sia insoddisfacibile. Per farlo, dobbiamo clausificare $\lnot F$ ed eseguire il metodo di risoluzione sull'insieme di clausole ottenuto. La clausificazione di $\lnot F$ produce il seguente insieme di clausole:
\[
...
\]
A questo punto applichiamo il metodo di risoluzione su $S$, che ricava facilmente la clausola vuota. Questo dimostra che $\lnot F$ è insoddisfacibile, e quindi $F$ è valida.


\section{La classe App}
\label{class:App}
L'implementazione di quanto visto in questo capitolo, viene fatto nella classe \texttt{App} del progetto Java per la logica classica e per le logiche modali. Come spiegato nella sezione \ref{project_structure}, la classe \texttt{App} contiene soltanto il metodo \texttt{main}. Il metodo \texttt{main} calcola la validità di una formula presa da \emph{Standard input}; esso esegue le seguenti operazioni:
